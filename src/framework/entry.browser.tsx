...
async function main() {
  ...
  function BrowserRoot() {
    const [payload, setPayload_] = React.useState<RscPayload>(initialPayload);

    React.useEffect(() => {
      // setPayload = (v) => React.startTransition(() => setPayload_(v))
      setPayload = (v) => React.startTransition(() => setPayload_((prev) => ({ ...prev, ...v })));
    }, [setPayload_]);
  ...
  setServerCallback(async (id, args) => {
    ...
    // Default case: update the current UI with the action's result
    setPayload(payload);
